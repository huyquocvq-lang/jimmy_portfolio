...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const isInternalLink = linkUrl && !linkUrl.startsWith('http') && !linkUrl.startsWith('#');

  return (
    <motion.div
      className={`bg-white flex flex-col items-start w-full h-full ${shadowClasses[shadowVariant]} rounded-lg md:rounded-xl overflow-hidden ${className}`}
      onClick={isInternalLink ? handleCardClick : undefined}
      role={isInternalLink ? 'button' : undefined}
      tabIndex={isInternalLink ? 0 : undefined}
      onKeyDown={(e) => {
        if (isInternalLink && (e.key === 'Enter' || e.key === ' ')) {
          e.preventDefault();
          handleCardClick();
        }
      }}
      ...
      <div className="flex flex-col gap-6 md:gap-8 items-start p-6 md:p-8 w-full">
        ...
        {linkUrl && (
          (linkUrl.startsWith('http') || linkUrl.startsWith('#')) ? (
            ...
            <div
              className="border-b border-[#5e3bee] flex gap-3 md:gap-5 items-center pb-2 md:pb-2.5 text-black hover:text-[#5e3bee] transition-colors pointer-events-none"
            >
              <span className="font-semibold text-sm md:text-base leading-[1.5]">{linkText}</span>
              <div className="relative shrink-0 w-3 h-3 md:w-4 md:h-4">
                <svg
