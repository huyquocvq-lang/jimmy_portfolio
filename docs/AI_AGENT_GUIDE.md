...
| Project preview images | `public/images/projects/*.jpg`, `scripts/generate-project-previews.mjs` | Low - shipped thumbnails should use approved real/reference visuals where available; generator is fallback for anonymized placeholders |
...
5. **Assets** - `public/images/projects/my-new-project.jpg` (fallback). Prefer an approved real/reference visual cropped to 1600×1000; use `node scripts/generate-project-previews.mjs --force` only when a project needs an anonymized placeholder. Optionally add `public/banners/my-new-project.html` (use any existing banner file as a template - keep the `.banner-fit` 560×510 wrapper and inline `--scale` script).
...
public/images/projects/*.jpg            1600x1000 project preview thumbnails
...
scripts/generate-project-previews.mjs   Generates anonymized project preview placeholders
